...
public final class DefaultMediaState: MediaState {
    public var simRadio: SimRadioMedia = .empty
    public var nonPersistedSimSeries: [SimGameSeries.ID] = []

    public var realRadio: RealRadioMedia = .empty
    public var nonPersistedRealStations: [RealStation.ID] = []

    public private(set) var downloadStatus: [MediaID: MediaDownloadStatus] = [:]
    ...
    public func mediaList(persisted: Bool) -> [MediaList] {
    ...
    public func addSimRadio(url: URL, persisted: Bool) async throws {
        try await simRadioLibrary.addSimRadio(url: url, persisted: persisted)
    }

    public func removeSimRadio(_ seriesID: SimGameSeries.ID) async throws {
    ...
    public func addRealRadio(_ stations: [RealStation], persisted: Bool) async throws {
        try await realRadioLibrary.addStations(stations, persisted: persisted)
    }

    public func removeRealRadio(_ id: RealStation.ID) async throws {
        try await realRadioLibrary.removeStation(withID: id)
    }

    public func download(_ mediaID: MediaID) async {
    ...
    public func removeDownload(_ mediaID: MediaID) async {
    ...
    public func pauseDownload(_ mediaID: MediaID) async {
        guard downloadStatus.keys.contains(mediaID) else { return }
...
extension DefaultMediaState {
    public var downloadedMedia: [Media] {
        downloadStatus
            .map(\.self)
...
extension DefaultMediaState: SimRadioMediaState {
    public var simDownloadStatus: [SimStation.ID: MediaDownloadStatus] {
