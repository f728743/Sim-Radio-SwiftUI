...
@MainActor
public final class DefaultRealRadioLibrary {
    public weak var mediaState: RealRadioMediaState?
    public weak var dataController: DataController?
    public weak var delegate: RealRadioLibraryDelegate?
...
extension DefaultRealRadioLibrary: RealRadioLibrary {
    ...
    public func addStations(_ stations: [RealStation], persisted: Bool) async throws {
        let new = RealRadioMedia(stations: Dictionary(uniqueKeysWithValues: stations.map { ($0.id, $0) }))
        addToLibrary(new, persisted: persisted)
    }

    public func removeStation(withID id: RealStation.ID) async throws {
