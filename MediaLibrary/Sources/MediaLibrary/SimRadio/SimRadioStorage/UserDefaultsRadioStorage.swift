...
@MainActor
public final class UserDefaultsRadioStorage: SimRadioStorage {
    ...
    public func addSeries(id: SimGameSeries.ID) {
    ...
    public func removeSeries(id: SimGameSeries.ID) {
        var currentIDs = addedSeriesIDs
        currentIDs.removeAll { $0 == id }
        saveSeriesIDs(currentIDs)
    }

    public func containsSeries(id: SimGameSeries.ID) -> Bool {
        addedSeriesIDs.contains(id)
    }

    public func setStorageState(_ state: StationStorageState, for stationID: SimStation.ID) {
    ...
    public func getStorageState(for stationID: SimStation.ID) -> StationStorageState? {
    ...
    public func removeStorageState(for stationID: SimStation.ID) {
        var currentStates = loadStoredStatesDictionary()
        currentStates.removeValue(forKey: stationID.userDefaultsKey)
        saveStoredStatesDictionary(currentStates)
    }

    public var addedSeriesIDs: [SimGameSeries.ID] {
    ...
    public var allStoredStationStates: [SimStation.ID: StationStorageState] {
